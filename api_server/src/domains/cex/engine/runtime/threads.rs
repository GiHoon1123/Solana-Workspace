...
fn handle_cancel_order(
    ...
    _wal_tx: &crossbeam::channel::Sender<WalEntry>,
    _orderbooks: &Arc<RwLock<HashMap<TradingPair, OrderBook>>>,
    _executor: &Arc<Mutex<Executor>>,
) {
    // TODO: 구현 (다음 단계에서)
    let _ = response.send(Err(anyhow::anyhow!("Not implemented yet")));
}
